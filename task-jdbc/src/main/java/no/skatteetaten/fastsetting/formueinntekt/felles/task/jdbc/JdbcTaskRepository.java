...
public class JdbcTaskRepository implements TaskRepository<Connection, SQLException> {
    ...
    public List<Task> push(
        ...
        try (PreparedStatement ps = conn.prepareStatement(
            "INSERT INTO TASK (TOPIC, IDENTIFIER, INPUT, REFERENCE, STATE, SCHEDULED) VALUES (?, ?, ?, ?, ?, CAST(CURRENT_TIMESTAMP AT TIME ZONE 'UTC' + ? AS TIMESTAMP))",
            new String[] {"SEQUENCE"}
        )) {
            for (TaskCreation creation : creations) {
                ...
                Duration duration = creation.getDuration().orElse(null);
                if (duration == null) {
    ...
    void doComplete(Connection conn, String topic, Map<Task, TaskDecision> decisions) throws SQLException {
        try (PreparedStatement ps = conn.prepareStatement(
            "UPDATE TASK "
                + "SET STATE = ?, DESCENT = GREATEST(STATE, DESCENT), OUTPUT = ?, COMPLETED = CURRENT_TIMESTAMP AT TIME ZONE 'UTC', SCHEDULED = CURRENT_TIMESTAMP AT TIME ZONE 'UTC' + ? "
                + "WHERE TOPIC = ? "
                + "AND SEQUENCE BETWEEN ? AND ? "
                + "AND STATE = ? "
                + "AND OWNER = ?"
            ...
            do {
                ...
                if (entry != null && entry.getValue().isSameAs(decision) && to == entry.getKey().getSequence() - 1) {
                    ...
                    Duration duration = decision.getDuration().orElse(null);
                    if (duration == null) {
                        ps.setNull(3, Types.NULL);
                    } else {
                        ps.setObject(3, duration);
                    }
                    ps.setString(4, topic);
                    ps.setLong(5, from);
                    ps.setLong(6, to);
                    ps.setInt(7, TaskState.ACTIVE.ordinal());
    ...
    @Override
    public long scheduled(Connection conn, String topic) throws SQLException {
        try (PreparedStatement ps = conn.prepareStatement(
            "UPDATE TASK "
                    + "SET STATE = ?, SCHEDULED = NULL "
                    + "WHERE TOPIC = ? "
                    + (concurrent ? "AND SEQUENCE IN (SELECT SEQUENCE FROM TASK WHERE TOPIC = ? " : "")
                    + "AND SCHEDULED < CAST(CURRENT_TIMESTAMP AT TIME ZONE 'UTC' AS TIMESTAMP) "
                    + (concurrent ? "FOR UPDATE SKIP LOCKED) " : "")
                    + "AND STATE BETWEEN ? AND ?"
        )) {
            ps.setInt(1, TaskState.READY.ordinal());
            ...
            if (concurrent) {
                ps.setString(3, topic);
                ps.setInt(4, TaskState.FAILED.ordinal());
                ps.setInt(5, TaskState.SUCCEEDED.ordinal());
            } else {
                ps.setInt(3, TaskState.FAILED.ordinal());
                ps.setInt(4, TaskState.SUCCEEDED.ordinal());
            }
            return ps.executeLargeUpdate();
        }
    }
    ...
    public List<TaskInfo> page(
        ...
        try (PreparedStatement ps = conn.prepareStatement(
            "SELECT SEQUENCE, IDENTIFIER, OWNER, REFERENCE, STATE, DESCENT, CREATED, COMPLETED, SCHEDULED, INPUT, OUTPUT "
                + "FROM TASK "
                + "WHERE TOPIC = ? "
            ...
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    Timestamp completed = rs.getTimestamp("COMPLETED"), scheduled = rs.getTimestamp("SCHEDULED");
                    tasks.add(new TaskInfo(
                        ...
                        scheduled == null ? null : scheduled.toLocalDateTime().atOffset(ZoneOffset.UTC),
