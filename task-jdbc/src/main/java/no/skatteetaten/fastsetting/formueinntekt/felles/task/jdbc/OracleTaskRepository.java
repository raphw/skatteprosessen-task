        ...
        if (concurrent) {
            try (CallableStatement cs = conn.prepareCall(
                "DECLARE "
                        ...
                        + "BEGIN "
                        + "SELECT SEQUENCE "
                        + "BULK COLLECT INTO SCHEDULE "
                        + "FROM TASK "
                        + "WHERE TOPIC = ? "
                        + "AND SCHEDULED < CAST(CURRENT_TIMESTAMP AT TIME ZONE 'UTC' AS TIMESTAMP) "
                        + "FOR UPDATE SKIP LOCKED; "
                        + "FORALL IDX IN 1 .. SCHEDULE.COUNT "
                        + "UPDATE TASK "
                        + "SET STATE = ?, SCHEDULED = NULL "
                        + "WHERE TOPIC = ? "
                        + "AND SEQUENCE = SCHEDULE(IDX) "
                        + "AND STATE BETWEEN ? AND ?; "
                        + "? := SQL%ROWCOUNT; "
                        + "END;"
            )) {
                cs.setString(1, topic);
                cs.setInt(2, TaskState.READY.ordinal());
                cs.setString(3, topic);
                cs.setInt(4, TaskState.FAILED.ordinal());
                cs.setInt(5, TaskState.SUCCEEDED.ordinal());
                cs.registerOutParameter(6, Types.INTEGER);
                cs.execute();
                return cs.getLong(6);
            }
        } else {
            return super.scheduled(conn, topic);
        }
    }
