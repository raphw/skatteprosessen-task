...
public class PostgresTaskRepository extends JdbcTaskRepository.WithIntervalMultiplication {
    ...
    public List<Task> push(
        ...
        do {
            ...
            do {
                try (PreparedStatement ps = conn.prepareStatement(
                        "INSERT INTO TASK (TOPIC, IDENTIFIER, STATE, INPUT, REFERENCE, SCHEDULED) "
                                + "VALUES " + String.join(", ", Collections.nCopies(bulkSize, "(?, ?, ?, ?, ?, CAST(CURRENT_TIMESTAMP AT TIME ZONE 'UTC' + ?::INTERVAL AS TIMESTAMP))")) + " "
                                + "RETURNING SEQUENCE"
                )) {
                    for (int index = 0; index < bulkSize; index++) {
                        ps.setString(6 * index + 1, topic);
                        ps.setString(6 * index + 2, remaining.get(index).getIdentifier());
                        ps.setInt(6 * index + 3, (duplicates == null || duplicates.computeIfPresent(remaining.get(index).getIdentifier(), (ignored, count) -> --count) == 0
                                ? (remaining.get(index).isSuspended() ? TaskState.SUSPENDED : TaskState.READY)
                                : TaskState.REDUNDANT).ordinal());
                        ps.setString(6 * index + 4, remaining.get(index).getInput().orElse(null));
                        ps.setString(6 * index + 5, remaining.get(index).getReference().orElse(null));
                        Duration duration = remaining.get(index).getDuration().orElse(null);
                        if (duration == null) {
                            ps.setNull(6 * index + 6, Types.NULL);
                        } else {
                            ps.setObject(6 * index + 6, duration, Types.OTHER);
                        }
                    }
