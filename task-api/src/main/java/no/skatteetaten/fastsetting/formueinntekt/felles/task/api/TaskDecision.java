...
public class TaskDecision {
    ...
    TaskDecision(TaskResult result, Throwable throwable, boolean continued, Duration duration) {
        this.result = result;
        this.throwable = throwable;
    ...
    public TaskDecision withContinuation(boolean continued) {
        return this.continued == continued ? this : new TaskDecision(result, throwable, continued, duration);
    }

    public TaskDecision withScheduling(Duration duration) {
        return Objects.equals(this.duration, duration) ? this : new TaskDecision(result, throwable, continued, duration);
    }
    ...
    public TaskDecision merge(TaskDecision other) {
        if (result == other.result && throwable == other.throwable) {
            ...
            return new TaskDecision(
                ...
                duration != null && (other.duration == null || duration.compareTo(other.duration) < 0) ? duration : other.duration
            );
        }
    }
    ...
    public boolean isSameAs(TaskDecision decision) {
        if (result != decision.result || !Objects.equals(duration, decision.duration)) {
            return false;
